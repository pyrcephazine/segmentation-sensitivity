...
def color_graph(
    ...
    if strategy is not None:
        ...
        undirected_colors = nx.coloring.greedy_color(undirected_edge_graph, strategy=strategy)
        edge_coloring = {
            (u,v): undirected_colors[(u,v)] if (u,v) in undirected_colors
            else undirected_colors[(v,u)] for u,v in graph.edges
        }
    else:
...
class NetworkMesh:
    ...
    def __init__(
        ...
        graph_rank: int = 0,
        cell_partitioner=None,
        build_submeshes: bool = True,
    ):
        self._build_mesh(
            graph, N=N, color_strategy=color_strategy, comm=comm, graph_rank=graph_rank,
            cell_partitioner=cell_partitioner,
        )
        if build_submeshes:
    ...
    def _build_mesh(
        ...
        if cell_partitioner is not None:
            part = cell_partitioner
        elif hasattr(_mesh, "create_cell_partitioner"):
    ...
    def in_edges(self, bifurcation_idx: int) -> npt.NDArray[np.int32]:
